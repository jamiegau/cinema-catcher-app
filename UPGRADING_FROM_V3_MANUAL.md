...
For normal upgrades use [the one-command upgrade](UPGRADING_FROM_V3.md).
The following is the historical supervised procedure, retained for custom
installations and recovery work. Do not mix its phases with a partially
completed automated upgrade without reviewing the saved state and backups.
...
old PostgreSQL service must be running when the dump is made. The helper now
defaults to `docker-compose.yml`; older copies used
`docker_compose_production.yml`. Always supply the explicit paths shown below.
